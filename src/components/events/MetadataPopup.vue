...
>
    <v-toolbar flat>
      <v-toolbar-title>Activity Feed</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-btn icon color="black" text @click="showPopup=false"><v-icon>mdi-close</v-icon></v-btn>
    </v-toolbar>
    <v-card-text style="overflow: auto; max-height: 385px; white-space: pre-wrap;">
...
<script>
import {watch, ref, onUnmounted, watchEffect} from 'vue';
...
export default {
  ...
  setup(props, {emit}) {
    ...
    watch(() => appStore.selectedEvent, () => {
      if(appStore.selectedEvent){
        codeBlock.value = null;
        // Based on whether the event has an event id or not, the key of the map that contains the metadata is either id or event_id
        if(appStore.selectedEvent.metadata.event.event_id){
          prettyJson.value = JSON.stringify(appStore.eventSourcesMap.get(appStore.selectedEvent.metadata.event.event_id), null, 2);
        }
        else {
          prettyJson.value = JSON.stringify(appStore.eventSourcesMap.get(appStore.selectedEvent.id), null, 2);
        }
